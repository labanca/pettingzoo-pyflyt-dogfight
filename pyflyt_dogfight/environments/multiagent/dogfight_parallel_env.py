...
class DogfightEnv(ParallelEnv):
    ...
    def __init__(
        ...
        act_space = spaces.Box(low=self.low, high=self.high, dtype=np.float64)

        obs_space = spaces.Box(
            low=-np.inf,
            high=np.inf,
            shape=(2 * self.state_shape + act_space.shape[0],)
        )

        self.action_spaces = dict(
            zip(
                self.agents,
                [act_space for _ in enumerate(self.agents)]
            )
        )
    ...
    def reset(self, seed=None, options=None): #-> tuple[np.ndarray, dict]:
        ...
        self.agent_drone_mapping = dict(zip(self.possible_agents, [drone.Id for drone in self.env.drones]))
    ...
    def step(self, actions): # -> tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray, dict]:
        ...
        print(f"{self.agents=}  self.drones.Ids:{[drone.Id for drone in self.env.drones]}, self.armed_drones.Ids:{[drone.Id for drone in self.env.armed_drones]}, {len(actions)=}")
        ...
        for _ in range(self.env_step_ratio):
            ...
            if any(value for value in self.terminations.values()) or any(value for value in self.truncations.values()):
                break
            # print(f"{self.agents=}  self.drones.Ids:{[drone.Id for drone in self.env.drones]}, self.armeddrones.Ids:{[drone.Id for drone in self.env.armed_drones]}")
            self.env.step()
            ...
            for agent, death in [
                (x[0], x[1] or y[1])
                for x, y in zip(self.terminations.items(), self.truncations.items())
            ]:
        ...
        for agent in self.agents:
            if self.render and not np.all(self.previous_hits[agent] == self.current_hits[agent]):
                ...
                self.env.changeVisualShape(
                    self.agent_drone_mapping[agent],
        ...
        # if any(self.truncations.values()):
        #    self.agents = []

        return self.state, self.reward, self.terminations, self.truncations, self.info

    def kill_agent(self, agent):
        self.agents.remove(agent)
        #self.env.drones.pop(self.env.drones[self.agent_id_mapping[agent]].Id - 1)
        #self.env.armed_drones.pop(self.env.armed_drones[self.agent_id_mapping[agent]].Id - 1)
        #del self.observation_spaces[agent]
        #del self.action_spaces[agent]
    ...
    def compute_term_trunc_reward(self):
        ...
        for i, agent in enumerate(self.agents):
            self.terminations[agent] |= out_of_bounds[self.agent_id_mapping[agent]]
            self.terminations[agent] |= self.health[agent] <= 0.0
            self.terminations[agent] |= collisions[self.agent_id_mapping[agent]]

            # truncations is just end
            self.truncations[agent] |= self.win_condition(agent)
            self.truncations[agent] |= self.step_count > self.max_steps
            ...
            self.reward[agent] -= 20.0 * self.current_hits[self.agents[self.num_agents-i-1]]
    ...
    def win_condition(self, specific_agent ):
        for agent, destroyed in self.terminations.items():
            if agent != specific_agent and not destroyed:
                return False
        return True

    def last_standing(self):
        count = 0
        n = len(self.terminations.values())

        for agent, is_destroyed in self.terminations.items():
            if not is_destroyed:
                count += 1
